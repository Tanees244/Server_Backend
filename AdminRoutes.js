    ...
    pool.query(
      ...
      (err, hotelDetailsResult) => {
        ...
        if (hotelDetailsResult.length > 0) {
          ...
          // Step 2: Select hotel_booking_id from hotel_booking using hotel_details_id
          pool.query(
            `SELECT hotel_booking_id FROM hotel_booking WHERE hotel_details_id = ?`,
            [hotelDetailsId],
            (err, bookingResult) => {
              if (err) {
                console.error("Error executing query:", err);
              ...
              if (bookingResult.length > 0) {
                const hotelBookingId = bookingResult[0].hotel_booking_id;
  
                // Step 3: Delete records from hotel_booking using hotel_booking_id
                pool.query(
                  `DELETE FROM hotel_booking WHERE hotel_booking_id = ?`,
                  [hotelBookingId],
                  (err) => {
                    if (err) {
                      console.error("Error executing query:", err);
                    ...
                    // Step 4: Continue with other deletion steps...
                    // Delete records from package_hotel_details using hotel_details_id
                    pool.query(
                      ...
                      (err) => {
                        ...
                        // Step 5: Delete records from hotel_details using given hotel_id
                        pool.query(
                          ...
                          (err) => {
                            ...
                            // Step 6: Delete records from hotel_vendor_personal_details using given hotel_id
                            pool.query(
                              ...
                              (err) => {
                                ...
                                // Step 7: Continue with other deletion steps...
                                // Delete records from vendors table using vendor_id
                                pool.query(
                                  ...
                                  (err, vendorResult) => {
                                    ...
                                    if (vendorResult.length > 0) {
                                      ...
                                      // Step 8: Delete records from hotels table using given hotel_id
                                      pool.query(
                                        ...
                                        (err) => {
                                          ...
                                          // Step 9: Continue with other deletion steps...
                                          // Delete records from users using user_id
                                          pool.query(
                                            `SELECT user_id FROM vendors WHERE vendor_id = ?`,
                                            [vendorId],
                                            (err, userResult) => {
                                              ...
                                              if (userResult.length > 0) {
                                                ...
                                                // Step 10: Delete records from vendors using vendor_id
                                                pool.query(
                                                  `DELETE FROM vendors WHERE vendor_id = ?`,
                                                  [vendorId],
                                                  (err) => {
                                                    ...
                                                    // Step 11: Delete records from users using user_id
                                                    pool.query(
                                                      `DELETE FROM users WHERE user_id = ?`,
                                                      [userId],
                                                      (err) => {
                ...
                console.error("No booking found for the provided hotelId");
